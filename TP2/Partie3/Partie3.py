...
def gen_plan_de_gestion(id_jeu_de_donnees):
    v = random.choice(["1.0", "2.0", "2.1"])
    
    doc = {
        "nom_fichier": f"PGD_{id_jeu_de_donnees}_v{v}.pdf",
        "chemin_fichier": f"/data/{id_jeu_de_donnees}/PGD_{id_jeu_de_donnees}_v{v}.pdf",
        "taille_fichier_octets": random.randint(100_000, 2_000_000),
        "type_mime": "application/pdf",
        "version_pgd": v,
        "modele_utilise": random.choice(["DMP OPIDoR v3", "Science Europe"])
    }
...
def gen_common_file(id_jeu_de_donnees, fname, mime, date_debut, date_fin):
    ...
    doc = {
        ...
        "taille_fichier_octets": random.randint(2_000, 150_000_000),
        "type_mime": mime,
        ...
        "annotations": [] if random.random() < 0.7 else [
            {
                ...
                "commentaire": random.choice([
                    "Valeur aberrante à vérifier",
                    "Donnée validée",
                    "Nécessite un traitement complémentaire",
                    "Anomalie détectée lors de l'acquisition"
                ])
            }
        ]
    }
...
def gen_csv(id_jeu_de_donnees, date_debut, date_fin):
    ...
    doc["taille_fichier_octets"] = random.randint(2_000, 600_000)

    colonnes = random.choice([
    ...
    doc["metadonnees_techniques"] = {
        ...
        "methode_analyse": random.choice(["Chromatographie", "Spectrophotométrie", "ISO", "Mesure électrochimique"])
    }
    return doc


def gen_png(id_jeu_de_donnees, date_debut, date_fin):
    fname = f"radiographie_{random.randint(1, 999):03d}.png"
    doc = gen_common_file(id_jeu_de_donnees, fname, "image/png", date_debut, date_fin)
    doc["taille_fichier_octets"] = random.randint(50_000, 9_000_000)

    largeur = random.choice([512, 1024, 2048, 4096, None])
    hauteur = largeur if largeur else random.choice([512, 1024, 2048, 4096])

    doc["metadonnees_techniques"] = {
        "type": "image",
        "largeur": largeur,
        "hauteur": hauteur,
        "resolution": random.choice(["72dpi", "150dpi", "300dpi"]),
        "id_patient_pseudo": f"P{random.randint(1, 999):03d}-{fake.lexify('???').upper()}",
        "type_examen": random.choice(["Radiographie Thoracique", "IRM Cérébrale", "Scanner Abdominal"])
    }
    return doc


def gen_wav(id_jeu_de_donnees, date_debut, date_fin):
    fname = f"entretien_{random.randint(1, 300):03d}.wav"
    doc = gen_common_file(id_jeu_de_donnees, fname, "audio/wav", date_debut, date_fin)
    doc["taille_fichier_octets"] = random.randint(500_000, 120_000_000)

    doc["metadonnees_techniques"] = {
        "type": "audio",
        "duree_sec": random.choice([None, random.randint(30, 3600)]),
        "frequence_echantillonnage_hz": random.choice([16000, 22050, 44100, 48000]),
        "canaux": random.choice([1, 2]),
        "langue": random.choice(["fr", "en", "it"]),
        "statut_anonymisation": random.choice(["full", "partial", "none"])
    }
    return doc


def gen_geotiff(id_jeu_de_donnees, date_debut, date_fin):
    fname = f"carte_{random.randint(1, 500):03d}.tiff"
    doc = gen_common_file(id_jeu_de_donnees, fname, "image/tiff", date_debut, date_fin)
    doc["taille_fichier_octets"] = random.randint(2_000_000, 400_000_000)

    doc["metadonnees_techniques"] = {
        "type": "geotiff",
        "largeur": random.choice([1024, 2048, 4096, None]),
        "hauteur": random.choice([1024, 2048, 4096, None]),
        "systeme_coordonnees": random.choice(["EPSG:4326 - WGS 84", "EPSG:3857"]),
        ...
        "resolution": random.choice(["1m/pixel", "10m/pixel", "30m/pixel"]),
        "bandes": random.choice([1, 3, 4, 8])
    }
    return doc


def gen_pdf(id_jeu_de_donnees, date_debut, date_fin):
    fname = f"document_{random.randint(1, 200):03d}.pdf"
    doc = gen_common_file(id_jeu_de_donnees, fname, "application/pdf", date_debut, date_fin)
    doc["taille_fichier_octets"] = random.randint(80_000, 8_000_000)

    doc["metadonnees_techniques"] = {
        "type": "pdf",
        "type_document": "Plan de Gestion de Données",
        "version_pgd": random.choice(["1.0", "2.1", "2.0"]),
        "modele_utilise": random.choice(["Science Europe", "DMP OPIDoR v3"])
    }
    return doc

# Génération d'un fichier aléatoire parmi les types possibles
def gen_file(id_jeu_de_donnees, date_debut, date_fin):
    ftype = random.choice(FILE_TYPES)  # proba équivalente
    if ftype == "csv":
...
def gen_dataset_doc(i):
    ...
    nb_files = random.randint(MIN_FILES, MAX_FILES)

    doc = {
        "id_jeu_de_donnees": id_jeu_de_donnees,
        "plan_de_gestion": gen_plan_de_gestion(id_jeu_de_donnees),
        "fichiers": [
            gen_file(id_jeu_de_donnees, date_debut, date_fin) for _ in range(nb_files)
        ]
    }
